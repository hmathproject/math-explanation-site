...
\\( \sin\theta = \dfrac{3}{5} \\)、\\( 0 < \theta < \dfrac{\pi}{2} \\) のとき、\\( \cos\theta \\) と \\( \tan\theta \\) を求めよ。
...
\\( 0 < \theta < \dfrac{\pi}{2} \\) より第1象限なので \\( \cos\theta > 0 \\)。
...
\\( \sin^2\theta + \cos^2\theta = \dfrac{9}{25} + \dfrac{16}{25} = \dfrac{25}{25} = 1 \\) ✓

---

## なぜ cos の符号を象限で決めるか

\\( \cos^2\theta = \dfrac{16}{25} \\) から \\( \cos\theta = \pm\dfrac{4}{5} \\) の2候補があります。\\( 0 < \theta < \dfrac{\pi}{2} \\)（第1象限）という条件があるから \\( \cos\theta > 0 \\) と確定できます。
