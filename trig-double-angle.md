...
\\( \sin\theta = \dfrac{2}{3} \\)、\\( \dfrac{\pi}{2} < \theta < \pi \\) のとき、\\( \sin 2\theta \\) と \\( \cos 2\theta \\) を求めよ。
...
\\( \dfrac{\pi}{2} < \theta < \pi \\)（第2象限）より \\( \cos\theta < 0 \\)。
...
\\( \sin^2 2\theta + \cos^2 2\theta = \dfrac{80}{81} + \dfrac{1}{81} = \dfrac{81}{81} = 1 \\) ✓

\\( 2\theta \\) の象限チェック：\\( \dfrac{\pi}{2} < \theta < \pi \\) なら \\( \pi < 2\theta < 2\pi \\)（第3・4象限）。\\( \sin 2\theta = -\dfrac{4\sqrt{5}}{9} < 0 \\) は第3・4象限と整合 ✓。\\( \cos 2\theta = \dfrac{1}{9} > 0 \\) は第4象限と整合 ✓。
