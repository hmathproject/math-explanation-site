...
## 数列記事の追加チェック（FP-20〜22）

数列カテゴリの記事・manuscript を書くときは以下を追加確認する：

- [ ] 等比数列の和: `r=1 のとき`と`r≠1 のとき`の 2 ケースを両方書いたか（FP-20）
- [ ] S_n から a_n を求める箇所: `n≥2 のとき`と明記し`a_1=S_1`の別確認を入れたか（FP-21）
- [ ] 図スクリプト: `ax.scatter` または `ax.bar` を使い連続曲線 `ax.plot` を使っていないか（FP-22）
- [ ] manuscript の右欄: display 数式のチェーンが 2行以上ある場合は分割ブロックに分けたか（FP-18）
- [ ] manuscript の plaintext 部に ≤, ≥, √, ∈ を直書きしていないか（FP-12）

## ベクトル記事の追加チェック（FP-23〜25）

ベクトルカテゴリの記事・manuscript を書くときは以下を追加確認する：

- [ ] 平行条件: 「非零ベクトルどうしなら平行 ⟺ 一方が他方の実数倍」と明記し、零ベクトルを除く理由を添えたか（FP-23）
- [ ] 垂直条件: `a·b=0` を公式として天下りにせず、`cos 90°=0` → 定義に代入 の流れで示したか（FP-24）
- [ ] 内積の成分公式: 余弦定理から導出したか（FP-24）
- [ ] manuscript の見出し（`###`）: `\overrightarrow{}` を使っていないか（FP-25）
  - 代替: `OA`, `AB`, `|AB|` など平文で書く
- [ ] manuscript の plaintext 部に ≤, ≥, √, ∈ を直書きしていないか（FP-12）
- [ ] 図スクリプト: Hiragino Sans にない特殊文字（⟺ U+27FA など）を使っていないか（代替: `=` や平文を使う）
- [ ] 右欄 display 数式のチェーンが 2行以上ある場合は分割ブロックに分けたか（FP-18）
